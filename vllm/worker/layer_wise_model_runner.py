...
from vllm.worker.model_runner import (
    ModelRunner, ModelInputForGPUWithSamplingMetadata, ModelInputForGPU,
    GPUModelRunnerBase,)
if TYPE_CHECKING:
...
TModelInputForGPU = TypeVar('TModelInputForGPU', bound="LayerWiseModelInputForGPU")

@dataclass(frozen=True)
class LayerWiseModelInputForGPU(ModelRunnerInputBase):
    """
    ...
    def as_broadcastable_tensor_dict(self) -> Dict[str, Any]:
        ...
        for attn_metadata in self.attn_metadata:
            if attn_metadata is not None:
                        _add_attn_metadata_broadcastable_dict(tensor_dict, attn_metadata)
        return tensor_dict
...
@dataclass(frozen=True)
class LayerWiseModelInputForGPUWithSamplingMetadata(LayerWiseModelInputForGPU):
    """
...
class LayerWiseModelInputForGPUBuilder(ModelRunnerInputBuilderBase[ModelInputForGPU]):
    ...
    class InterDataForSeqGroup:
        ...
        def __init__(
            ...
            *,
            ...
            # layer_block_tables: Optional[Dict[int, Dict[int, List[int]]]],
    ...
    def gen_inter_data_builder(self, num_seqs: int):
        return lambda: LayerWiseModelInputForGPUBuilder.InterDataForSeqGroup(
            request_id="",
            seq_ids=[0] * num_seqs,
            is_prompt=True,
    ...
    def __init__(self,
        ...
        self.inter_data_list: Dict[int, List[
            LayerWiseModelInputForGPUBuilder.InterDataForSeqGroup]] = {_:[] for _ in range(num_layers)}

        # Attention metadata inputs.
        # self.attn_metadata_builder = {_:self.attn_backend.make_metadata_builder(
        #     weakref.proxy(self)) for _ in range(num_layers)}
    ...
    def add_seq_group(self, seq_group_metadata: SequenceGroupMetadata):
        ...
        for layer_id in range(self.num_layers):
            if seq_group_metadata.layer_block_tables is None:
                block_tables = None
            else:
                block_tables =  {
                    seq_id: layer_table.get(layer_id, [])
                    for seq_id, layer_table in seq_group_metadata.layer_block_tables.items()
                }
            inter_data_cur_layer = self.init_cached_inter_data(
            ...
            self.inter_data_list[layer_id].append(inter_data_cur_layer)
            for seq_idx in range(n_seqs):
                for per_seq_fn in self.per_seq_compute_fns:
                    per_seq_fn(inter_data_cur_layer, seq_idx, seq_group_metadata)
            for per_seq_group_fn in self.per_seq_group_compute_fns:
                per_seq_group_fn(inter_data_cur_layer, seq_group_metadata)
    ...
    def build(self) -> ModelInputForGPU:
        ...
        token_types = []
        attn_metadatas = []
        inter_data_list = self.inter_data_list[0]
        ...
        # Attention metadata.
        for layer_id in range(self.num_layers):
            attn_metadata = self.attn_metadata_builder[layer_id].build(
                seq_lens, query_lens, cuda_graph_pad_size, batch_size,
                inter_data_list=self.inter_data_list[layer_id])
            attn_metadatas.append(attn_metadata)
        
        # attn_metadata = self.attn_metadata_builder.build(
        #     seq_lens, query_lens, cuda_graph_pad_size, batch_size,
        #     inter_data_list=inter_data_list)
        ...
        lora_mapping = None
        if self.enable_lora:
            lora_requests = set(r for data in inter_data_list
        ...
        if self.enable_prompt_adapter:
            prompt_adapter_requests = set(
                data.prompt_adapter_request for data in inter_data_list
        ...
        multi_modal_kwargs_list = [
            data.multi_modal_kwargs for data in inter_data_list
...
class LayerWiseModelRunner(ModelRunner):
    _model_input_cls: Type[LayerWiseModelInputForGPUWithSamplingMetadata] = (
    ...
    def _prepare_model_input_tensors(
        ...
        builder = self._builder_cls(weakref.proxy(self), num_layers, finished_requests_ids)
    ...
    def execute_model(
        ...
        prefill_meta = model_input.attn_metadata[0].prefill_metadata
        decode_meta = model_input.attn_metadata[0].decode_metadata
