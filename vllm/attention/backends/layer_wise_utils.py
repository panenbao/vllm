...
def layer_wise_compute_slot_mapping(is_profile_run: bool, slot_mapping: Dict[int, List[int]],
                         seq_id: int, seq_len: int, context_len: int,
                         start_idx: int, block_size: int,
                         block_tables: Dict[int, Dict[int, List[int]]]):
    """
    Compute slot mapping for each layer.
    """
    if is_profile_run:
        ...
        for layer_id in block_tables.keys():
            if layer_id not in slot_mapping:
                slot_mapping[layer_id] = []
            slot_mapping[layer_id].extend([PAD_SLOT_ID] * seq_len)
        return
    ...
    for layer_id, layer_block_table in block_tables.items():
        if layer_id not in slot_mapping:
            slot_mapping[layer_id] = []
        slot_mapping[layer_id].extend([PAD_SLOT_ID] * padding_mask_len)
        ...
        if numel < _COMPUTE_SLOT_MAPPING_NUMPY_NUMEL:
            _compute_slot_mapping_python(slot_mapping[layer_id], block_table,
                                         range_start, range_end, block_size)
        else:
            _compute_slot_mapping_numpy(slot_mapping[layer_id], block_table,
                                        range_start, range_end, block_size)
